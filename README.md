...
관리자는 아래 화면으로 분리된 흐름에서 인터뷰를 운영할 수 있습니다.

- `/admin`: 인터뷰 목록
- `/admin/interviews/new`: 인터뷰 생성
- `/admin/interviews/[id]`: 인터뷰 상세/수정/활성화/삭제

생성/상세 화면에서 아래 항목을 설정할 수 있습니다.

- 인터뷰 제목
- 인터뷰 기간(시작/종료 시각)
- 뽑고 싶은 인재 정보(요구 역량)
- 접근 방식
  - `비밀번호`: 인터뷰 접근 비밀번호를 설정
  - `지정된 인원`: 특정 `interviewee` 계정만 접근 허용
